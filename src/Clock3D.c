...
SegmentDisplay3D    *clock_days_leftDigitB ;
SegmentDisplay3D    *clock_days_rightDigitA ;
SegmentDisplay3D    *clock_days_rightDigitB ;
...
SegmentDisplay3D    *clock_hours_leftDigitA ;
SegmentDisplay3D    *clock_hours_leftDigitB ;
SegmentDisplay3D    *clock_hours_rightDigitA ;
SegmentDisplay3D    *clock_hours_rightDigitB ;
...
SegmentDisplay3D    *clock_minutes_leftDigitA ;
SegmentDisplay3D    *clock_minutes_leftDigitB ;
SegmentDisplay3D    *clock_minutes_rightDigitA ;
SegmentDisplay3D    *clock_minutes_rightDigitB ;
...
SegmentDisplay3D    *clock_seconds_leftDigit ;
SegmentDisplay3D    *clock_seconds_rightDigit ;
RadialDial3D        *clock_seconds_radial ;

static int8_t        clock_second100ths      = -1 ;  // Initialy undefined.
SegmentDisplay3D    *clock_second100ths_leftDigit ;
SegmentDisplay3D    *clock_second100ths_rightDigit ;
...
( )
{
  ...
  clock_days_leftDigitA         = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_days_leftDigitB         = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_days_rightDigitA        = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_days_rightDigitB        = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;

  clock_hours_leftDigitA        = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_hours_leftDigitB        = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_hours_rightDigitA       = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_hours_rightDigitB       = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_hours_radial            = RadialDial3D_new( RADIAL_DIAL_TYPE_24 ) ;

  clock_minutes_leftDigitA      = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_minutes_leftDigitB      = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_minutes_rightDigitA     = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_minutes_rightDigitB     = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_minutes_radial          = RadialDial3D_new( RADIAL_DIAL_TYPE_60 ) ;

  clock_seconds_leftDigit       = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_seconds_rightDigit      = SegmentDisplay3D_new( CLOCK3D_DISPLAY_TYPE_MAJOR ) ;
  clock_seconds_radial          = RadialDial3D_new( RADIAL_DIAL_TYPE_60 ) ;

  clock_second100ths_leftDigit  = SegmentDisplay3D_new( CLOCK3D_SECOND100THS_DISPLAYTYPE_MAJOR ) ;
  clock_second100ths_rightDigit = SegmentDisplay3D_new( CLOCK3D_SECOND100THS_DISPLAYTYPE_MAJOR ) ;
...
#ifdef QEMU
  clock_hours_radial->mesh->strokeWidthAlternate = 1 ;
  clock_hours_radial->mesh->strokeColorAlternate = GColorWhite ;
#else
...
#ifdef QEMU
  clock_minutes_radial->mesh->strokeWidthAlternate = 1 ;
  clock_minutes_radial->mesh->strokeColorAlternate = GColorWhite ;
#else
...
#ifdef QEMU
  clock_seconds_radial->mesh->strokeWidthAlternate = 1 ;
  clock_seconds_radial->mesh->strokeColorAlternate = GColorWhite ;
#else
...
#ifdef QEMU
  clock_second100ths_radial->mesh->strokeColorAlternate = GColorWhite ;
#else
...
( )
{
  ...
  free( SegmentDisplay3D_free( clock_days_rightDigitB    ) )     ; clock_days_rightDigitB        = NULL ;
  free( SegmentDisplay3D_free( clock_days_rightDigitA    ) )     ; clock_days_rightDigitA        = NULL ;

  free( SegmentDisplay3D_free( clock_hours_leftDigitB    ) )     ; clock_hours_leftDigitB        = NULL ;
  free( SegmentDisplay3D_free( clock_hours_leftDigitA    ) )     ; clock_hours_leftDigitA        = NULL ;
  free( SegmentDisplay3D_free( clock_hours_rightDigitB   ) )     ; clock_hours_rightDigitB       = NULL ;
  free( SegmentDisplay3D_free( clock_hours_rightDigitA   ) )     ; clock_hours_rightDigitA       = NULL ;
  free( RadialDial3D_free    ( clock_hours_radial        ) )     ; clock_hours_radial            = NULL ;

  free( SegmentDisplay3D_free( clock_minutes_leftDigitB  ) )     ; clock_minutes_leftDigitB      = NULL ;
  free( SegmentDisplay3D_free( clock_minutes_leftDigitA  ) )     ; clock_minutes_leftDigitA      = NULL ;
  free( SegmentDisplay3D_free( clock_minutes_rightDigitB ) )     ; clock_minutes_rightDigitB     = NULL ;
  free( SegmentDisplay3D_free( clock_minutes_rightDigitA ) )     ; clock_minutes_rightDigitA     = NULL ;
  free( RadialDial3D_free    ( clock_minutes_radial      ) )     ; clock_minutes_radial          = NULL ;

  free( SegmentDisplay3D_free( clock_seconds_leftDigit   ) )     ; clock_seconds_leftDigit       = NULL ;
  free( SegmentDisplay3D_free( clock_seconds_rightDigit  ) )     ; clock_seconds_rightDigit      = NULL ;
  free( RadialDial3D_free    ( clock_seconds_radial      ) )     ; clock_seconds_radial          = NULL ;

  free( SegmentDisplay3D_free( clock_second100ths_leftDigit  ) ) ; clock_second100ths_leftDigit  = NULL ;
  free( SegmentDisplay3D_free( clock_second100ths_rightDigit ) ) ; clock_second100ths_rightDigit = NULL ;
...
( const int                pAnimStep
, const SegmentDisplayType pDisplayType
)
{
  ...
  SegmentDisplay3D_config( clock_days_leftDigitA
                         , pDisplayType
                         , digitWidth                               // width
                         , digitHeight                              // height
  ...
  SegmentDisplay3D_config( clock_days_leftDigitB
                         , pDisplayType
                         , digitWidth                               // width
                         , digitHeight                              // height
                         , -digitShift                              // shift left
...
)
{
  ...
  SegmentDisplay3D_config( clock_days_rightDigitA
                         , pDisplayType
                         , digitWidth                               // width
                         , digitHeight                              // height
  ...
  SegmentDisplay3D_config( clock_days_rightDigitB
                         , clock_displayType
                         , digitWidth                               // width
                         , digitHeight                              // height
...
)
{
  ...
  SegmentDisplay3D_config( clock_hours_leftDigitA
                         , pDisplayType
                         , digitWidth                                // width
                         , digitHeight                               // height
                         ,-digitShift                                // shift left
  ...
  SegmentDisplay3D_config( clock_hours_leftDigitB
                         , pDisplayType
                         , digitWidth                                // width
                         , digitHeight                               // height
                         ,-digitShift                                // shift left
...
)
{
  ...
  SegmentDisplay3D_config( clock_hours_rightDigitA
                         , pDisplayType
                         , digitWidth                                // width
                         , digitHeight                               // height
  ...
  SegmentDisplay3D_config( clock_hours_rightDigitB
                         , pDisplayType
                         , digitWidth                                // width
                         , digitHeight                               // height
...
)
{
  ...
  SegmentDisplay3D_config( clock_minutes_leftDigitA
                         , pDisplayType
                         , digitWidth                                // width
                         , digitHeight                               // height
                         ,-digitShift                                // shift left
  ...
  SegmentDisplay3D_config( clock_minutes_leftDigitB
                         , pDisplayType
                         , digitWidth                                // width
                         , digitHeight                               // height
                         ,-digitShift                                // shift left
...
)
{
  ...
  SegmentDisplay3D_config( clock_minutes_rightDigitA
                         , pDisplayType
                         , digitWidth                                               // width
                         , digitHeight                                              // height
  ...
  SegmentDisplay3D_config( clock_minutes_rightDigitB
                         , pDisplayType
                         , digitWidth                                               // width
                         , digitHeight                                              // height
                         , digitShift                                               // shift right
...
( const SegmentDisplayType pDisplayType )
{
    ...
    SegmentDisplay3D_config( clock_seconds_leftDigit
                           , pDisplayType
                           , digitWidth                        // width
                           , digitHeight                       // height
                           ,-digitShift                        // shift left
...
( const SegmentDisplayType pDisplayType )
{
    ...
    SegmentDisplay3D_config( clock_seconds_rightDigit
                           , pDisplayType
                           , digitWidth                        // width
                           , digitHeight                       // height
                           , digitShift                        // shift right
...
( )
{
  ...
  SegmentDisplay3D_config( clock_second100ths_leftDigit
...
( )
{
  ...
  SegmentDisplay3D_config( clock_second100ths_rightDigit
                         , CLOCK3D_SECOND100THS_DISPLAYTYPE
                         , digitWidth                        // width
                         , digitHeight                       // height
                         , digitShift                        // shift right
...
( const int pMaxAnimSteps )
{
  ...
  {
    SegmentDisplay3D_setValueInt( clock_days_leftDigitA, clock_days_leftDigitB->value ) ;              // A side inherits B side value.
  ...
  {
    SegmentDisplay3D_setValueInt( clock_days_rightDigitA, clock_days_rightDigitB->value ) ;              // A side inherits B side value.
  ...
  {
    SegmentDisplay3D_setValueInt( clock_hours_leftDigitA, clock_hours_leftDigitB->value ) ;              // A side inherits B side value.
  ...
  {
    SegmentDisplay3D_setValueInt( clock_hours_rightDigitA, clock_hours_rightDigitB->value ) ;     // A side inherits B side value.
  ...
  {
    SegmentDisplay3D_setValueInt( clock_minutes_leftDigitA, clock_minutes_leftDigitB->value ) ;     // A side inherits B side value.
  ...
  {
    SegmentDisplay3D_setValueInt( clock_minutes_rightDigitA, clock_minutes_rightDigitB->value ) ;     // A side inherits B side value.
...
( )
{
  if (clock_days_leftDigit_animStep == 0)
  {
    SegmentDisplay3D_setValueInt( clock_days_leftDigitB, clock_days_leftDigitA->value ) ;              // B side inherits A side value.
  ...
  if (clock_days_rightDigit_animStep == 0)
  {
    SegmentDisplay3D_setValueInt( clock_days_rightDigitB, clock_days_rightDigitA->value ) ;              // B side inherits A side value.
  ...
  if (clock_hours_leftDigit_animStep == 0)
  {
    SegmentDisplay3D_setValueInt( clock_hours_leftDigitB, clock_hours_leftDigitA->value ) ;              // B side inherits A side value.
  ...
  if (clock_hours_rightDigit_animStep == 0)
  {
    SegmentDisplay3D_setValueInt( clock_hours_rightDigitB, clock_hours_rightDigitA->value ) ;              // B side inherits A side value.
  ...
  if (clock_minutes_leftDigit_animStep == 0)
  {
    SegmentDisplay3D_setValueInt( clock_minutes_leftDigitB, clock_minutes_leftDigitA->value ) ;              // B side inherits A side value.
  ...
  if (clock_minutes_rightDigit_animStep == 0)
  {
    SegmentDisplay3D_setValueInt( clock_minutes_rightDigitB, clock_minutes_rightDigitA->value ) ;              // B side inherits A side value.
...
void
Clock3D_setDisplayType( SegmentDisplayType pDisplayType )
{
...
Clock3D_cycleDisplayType( )
{
  switch (clock_displayType)
  {
    case SEG_NCURVYBONESKIN:
      Clock3D_setDisplayType( SEG_NCURVYSKINBONE ) ;
      break ;

    case SEG_NCURVYSKINBONE:
      Clock3D_setDisplayType( SEG_NCURVYSKIN ) ;
      break ;

    case SEG_NCURVYSKIN:
      Clock3D_setDisplayType( SEG_NCURVYBONE ) ;
      break ;

    case SEG_NCURVYBONE:
      Clock3D_setDisplayType( SEG_N7SKINBONE ) ;
      break ;

    case SEG_N7SKINBONE:
      Clock3D_setDisplayType( SEG_N7BONESKIN ) ;
      break ;

    case SEG_N7BONESKIN:
      Clock3D_setDisplayType( SEG_N7SKIN ) ;
      break ;

    case SEG_N7SKIN:
      Clock3D_setDisplayType( SEG_N7BONE ) ;
      break ;
...
( const uint8_t pDays )
{
  ...
  if (isUninitialized)
  {
    SegmentDisplay3D_setValueInt( clock_days_leftDigitA, days_leftDigit  ) ;
    clock_days_leftDigitA->mesh->state.isDisabled = (clock_days_leftDigitA->value == 0) ;     // Show A side (if not zero).

    SegmentDisplay3D_setValueInt( clock_days_leftDigitB, days_leftDigit  ) ;
    clock_days_leftDigitB->mesh->state.isDisabled = true ;                                    // Hide B side.

    SegmentDisplay3D_setValueInt( clock_days_rightDigitA, days_rightDigit ) ;
    clock_days_rightDigitA->mesh->state.isDisabled = false ;                                  // Show A side

    SegmentDisplay3D_setValueInt( clock_days_rightDigitB, days_rightDigit ) ;
  ...
  else
  {
    ...
    {
      SegmentDisplay3D_setValueInt( clock_days_leftDigitB, days_leftDigit ) ;                 // Set B side value with new digit value.
    ...
    {
      SegmentDisplay3D_setValueInt( clock_days_rightDigitB, days_rightDigit ) ;   // Set B side value with new digit value.
...
( const uint8_t pHours )
{
  ...
  if (isUninitialized)
  {
    SegmentDisplay3D_setValueInt( clock_hours_leftDigitA, hours_leftDigit  ) ;
    clock_hours_leftDigitA->mesh->state.isDisabled = (clock_hours_leftDigitA->value == 0) ;   // Show A side (if not zero).

    SegmentDisplay3D_setValueInt( clock_hours_leftDigitB, hours_leftDigit  ) ;
    clock_hours_leftDigitB->mesh->state.isDisabled = true ;                                   // Hide B side.

    SegmentDisplay3D_setValueInt( clock_hours_rightDigitA, hours_rightDigit ) ;
    clock_hours_rightDigitA->mesh->state.isDisabled = false ;                                 // Show A side

    SegmentDisplay3D_setValueInt( clock_hours_rightDigitB, hours_rightDigit ) ;
  ...
  else
  {
    ...
    {
      SegmentDisplay3D_setValueInt( clock_hours_leftDigitB, hours_leftDigit ) ;                 // Set B side value with new digit value.
    ...
    {
      SegmentDisplay3D_setValueInt( clock_hours_rightDigitB, hours_rightDigit ) ;   // Set B side value with new digit value.
...
( const uint8_t pMinutes )
{
  ...
  {
    Clock3D_minutes_leftDigit_config( clock_minutes_leftDigit_animStep, clock_displayType  ) ;
    SegmentDisplay3D_setValueInt( clock_minutes_leftDigitA , minutes_leftDigit  ) ;
    ...
    Clock3D_minutes_rightDigit_config( clock_minutes_rightDigit_animStep, clock_displayType ) ;
    SegmentDisplay3D_setValueInt( clock_minutes_rightDigitA, minutes_rightDigit ) ;
  ...
  {
    SegmentDisplay3D_setValueInt( clock_minutes_leftDigitB, minutes_leftDigit ) ;   // Set B side value with new digit value.
  ...
  {
    SegmentDisplay3D_setValueInt( clock_minutes_rightDigitB, minutes_rightDigit ) ;   // Set B side value with new digit value.
...
( const uint8_t pSeconds )
{
  ...
  SegmentDisplay3D_setValueInt( clock_seconds_leftDigit , clock_seconds / 10 ) ;
  SegmentDisplay3D_setValueInt( clock_seconds_rightDigit, clock_seconds % 10 ) ;
...
( const uint8_t pSecond100ths )
{
  ...
  SegmentDisplay3D_setValueInt( clock_second100ths_leftDigit , clock_second100ths / 10 ) ;   // 1/10 th of a second.
  SegmentDisplay3D_setValueInt( clock_second100ths_rightDigit, clock_second100ths % 10 ) ;   // 1/100 th of a second.
